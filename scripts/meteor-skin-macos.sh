...
usage() {
  cat <<'EOF'
Meteor Skin for macOS

Usage:
  scripts/meteor-skin-macos.sh install [options]
  scripts/meteor-skin-macos.sh start [start options]
  scripts/meteor-skin-macos.sh quick-theme <image> [--name NAME] [--layout LAYOUT]
  scripts/meteor-skin-macos.sh theme <name> [banner|fullscreen] [--port PORT]
  scripts/meteor-skin-macos.sh verify [verify options]
  scripts/meteor-skin-macos.sh doctor [--port PORT] [--app PATH]
  scripts/meteor-skin-macos.sh restore [restore options]
  scripts/meteor-skin-macos.sh uninstall [--yes] [--port PORT] [--app PATH]
...
case "$COMMAND" in
  ...
  install)
    ...
    INSTALLED_SCRIPT="$(dream_state_root)/runtime/scripts/meteor-skin-macos.sh"

    if [ "$NO_START" -eq 1 ]; then
      echo "Meteor Skin is installed. Run '$INSTALLED_SCRIPT start' when you are ready to launch it."
      exit 0
    fi

    if ! dream_cdp_ready "$PORT" && [ -n "$(dream_main_pids)" ] && [ "$RESTART_EXISTING" -ne 1 ]; then
      if [ -t 0 ]; then
        printf '\nCodex is currently open. Restart it now to enable Meteor Skin? [y/N] '
        read -r answer
        case "$answer" in
          y|Y|yes|YES|Yes) RESTART_EXISTING=1 ;;
          *)
            echo "Meteor Skin is installed but has not restarted Codex."
            echo "Quit Codex later, then run: $INSTALLED_SCRIPT start"
            exit 0
            ;;
        esac
      else
        echo "Meteor Skin is installed, but Codex is already open and was not restarted."
    ...
    echo "Meteor Skin installation is complete."
    ;;
  ...
  uninstall)
    ...
    if [ "$YES" -ne 1 ]; then
      ...
      printf 'Remove Meteor Skin and restore the pre-install Codex colors? [y/N] '
      read -r answer
      case "$answer" in
        y|Y|yes|YES|Yes) ;;
